...
def test_user(sync_connection, unique_name):
    ...
    resp = sync_connection.invoke(HttpMethods.POST, Services.CORE, "Users", body=user_body)
...
class TestUserRead:
    def test_read_created_user(self, sync_connection, test_user):
        resp = sync_connection.invoke(HttpMethods.GET, Services.CORE, f"Users/{test_user['Id']}")
...
class TestUserUpdate:
    def test_update_description(self, sync_connection, test_user):
        ...
        resp = sync_connection.invoke(HttpMethods.PUT, Services.CORE, f"Users/{test_user['Id']}", body=updated)
        assert resp.status_code == 200
...
class TestUserSetPassword:
    def test_set_password(self, sync_connection, test_user):
        resp = sync_connection.invoke(HttpMethods.PUT, Services.CORE, f"Users/{test_user['Id']}/Password", body="TestP@ssw0rd123!")
