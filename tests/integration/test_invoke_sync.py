...
class TestInvokeGet:
    def test_get_users(self, sync_connection):
        resp = sync_connection.invoke(HttpMethods.GET, Services.CORE, "Users")
    ...
    def test_get_with_query_filter(self, sync_connection):
        ...
        resp = sync_connection.invoke(HttpMethods.GET, Services.CORE, "Users", query={"filter": "Disabled eq false"})
        assert resp.status_code == 200

    def test_get_appliance_status(self, sync_connection):
        resp = sync_connection.invoke(HttpMethods.GET, Services.APPLIANCE, "SystemTime")
        assert resp.status_code == 200
...
class TestInvokePostPutDelete:
    def test_crud_lifecycle(self, sync_connection, unique_name):
        ...
        # CREATE
        create_resp = sync_connection.invoke(HttpMethods.POST, Services.CORE, "Users", body=user_body)
        ...
        try:
            # READ
            get_resp = sync_connection.invoke(HttpMethods.GET, Services.CORE, f"Users/{user_id}")
            assert get_resp.status_code == 200
            assert get_resp.json()["Name"] == unique_name

            # UPDATE
            update_body = {**user_data, "Description": "Updated by PySafeguard integration test"}
            put_resp = sync_connection.invoke(HttpMethods.PUT, Services.CORE, f"Users/{user_id}", body=update_body)
            assert put_resp.status_code == 200
            assert put_resp.json()["Description"] == "Updated by PySafeguard integration test"
        finally:
            # DELETE (cleanup)
            delete_resp = sync_connection.invoke(HttpMethods.DELETE, Services.CORE, f"Users/{user_id}")
...
class TestInvokeWithAdditionalHeaders:
    def test_custom_header_passthrough(self, sync_connection):
        ...
        resp = sync_connection.invoke(HttpMethods.GET, Services.CORE, "Me", additionalHeaders={"X-Test": "pytest"})
        assert resp.status_code == 200
